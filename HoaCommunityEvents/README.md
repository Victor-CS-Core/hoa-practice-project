...
- The preflight proves that the App Service name, slot-scoped publish profile, and canonical non-production slot are configured before database mutation can begin. The release SHA and exact trusted CI run are revalidated after every approval and again immediately before database mutation or staging deployment.
- Configuration and freshness checks do not prove that the live Azure slot exists or is correctly owned. Live resource ownership and readiness remain an external Azure gate.
