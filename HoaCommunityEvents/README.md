...
- Manual runs remain available through **Actions → Deploy HOA BFF to Azure App Service → Run workflow**. Select `main`; the current `main` commit must already have a successful push-triggered `CI` run. Manual dispatch does not bypass exact-CI verification or any protected GitHub Environment job.
- The workflow is an explicit chain: unprotected artifact preparation and verification, a non-mutating `production`-protected staging configuration preflight, the separate `production-database` approval, then the actual `production`-protected staging deployment. GitHub evaluates an Environment's protection rules before each job that references it is sent to a runner, so both `production` jobs must pass those rules independently and the intervening database approval remains distinct.
- The preflight parses the publish-profile XML without logging it and mirrors Azure's deployment-action identity rule: remove a leading `$` from the deployment username, compare case-insensitively, split on `__`, and require segment 0 to match the App Service name and segment 1 to match the canonical non-production slot. That structural identity check must pass before database mutation can begin. The release SHA and exact trusted CI run are revalidated after every approval and again immediately before database mutation or staging deployment.
- The publish-profile identity, configuration, and freshness checks do not prove that the live Azure subscription/resource is owned by the operator or that the named slot exists. Live Azure ownership, slot existence, and readiness remain an external gate.
...
| `production` | `AZURE_WEBAPP_PUBLISH_PROFILE_PRODUCTION` | Nonempty XML secret containing a publish profile for the configured App Service staging slot. Protected preflight and deploy validation compare its deployment username identity to the configured app and canonical slot without printing the secret. |
...
The migration secret does not configure the application's runtime database connection. App Service still needs its own `ConnectionStrings__DefaultConnection`, Cloudinary settings, Production environment, and .NET 10 runtime. Keep `Seed__EnableBootstrap` and `Seed__EnableDemoData` false. Keep one API instance until the process-local SSE broker is replaced with a shared backplane. In workflow migration mode, restore/build completes in Release before the final freshness check, and the secret-bearing EF command uses `--no-build --configuration Release`. CI has exactly `contents: read` at workflow scope with no job override; each deployment job grants only the read permission it needs, while the publish profile provides separate Azure staging authority.

**Release gate:** once these workflow changes are pushed to GitHub's default branch, a successful trusted `main` CI run preserves the exact artifact before any database action. The first `production` approval protects the non-mutating configuration preflight; the distinct `production-database` approval then gates database responsibility; and the second `production`-protected job gates staging deployment. Freshness is rechecked after approvals and directly before either mutation. Before staging, externally confirm the live target, slot ownership, secrets, database backup/migration readiness, and release authority. A human-authorized slot swap and the full authenticated smoke matrix complete production cutover. Coordinate BFF login/current/logout, role, CSRF, deep-link, Cloudinary, database, and SSE smoke tests before switching the client-facing domain. Publishing source changes is not itself a slot swap, DNS change, or retirement of the old Azure resource.
