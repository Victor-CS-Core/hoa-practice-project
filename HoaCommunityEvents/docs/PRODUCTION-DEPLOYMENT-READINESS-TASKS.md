...
- Confirm the staging preflight rejects missing/malformed profile XML, a wrong app, a wrong slot, and any production target before `production-database` becomes eligible. This is a structural publish-profile identity check; it does not prove Azure subscription/resource ownership or that the live slot exists. Verify both through the separate Azure ownership/readiness gate.
...
- Do not begin either path until the protected staging configuration preflight has succeeded. In workflow mode, restore and build the EF startup project in Release without the SQL secret, then reconfirm current `main` and the exact trusted CI run immediately before the secret-bearing `dotnet ef database update --configuration Release --no-build` step.
...
- [Azure/webapps-deploy publish-profile app and slot validation](https://github.com/Azure/webapps-deploy/blob/master/src/ActionInputValidator/Validations.ts)
