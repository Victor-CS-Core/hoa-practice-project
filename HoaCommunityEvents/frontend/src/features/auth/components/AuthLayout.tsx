      ...
      <div className="mx-auto grid min-h-[calc(100svh-9rem)] max-w-6xl grid-cols-1 gap-5 p-4 sm:p-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-8 lg:p-10">
        <div className="auth-motion-board relative overflow-hidden rounded-4xl border border-hairline bg-surface p-6 text-ink-body shadow-[0_25px_60px_-30px_rgba(15,23,42,0.28)] sm:p-8 lg:p-10">
          ...
          <div className="relative z-10">
            <p className="font-auth-ui auth-motion-item w-fit rounded-full border border-accent/35 bg-accent-faded px-3 py-1 text-xs uppercase tracking-[0.22em] text-accent-display">
              {BRAND.communityLabel}
            </p>
            <h1 className="font-auth-display auth-motion-item auth-motion-delay-1 mt-5 max-w-lg text-3xl font-semibold leading-tight text-ink-display sm:text-4xl lg:text-5xl">
              Built for front porches, block parties, and real neighbors.
            </h1>
            <p className="auth-motion-item auth-motion-delay-2 mt-4 max-w-xl text-base leading-relaxed text-ink-muted sm:text-lg">
            ...
            <div className="auth-motion-item auth-motion-delay-3 mt-7 grid grid-cols-2 gap-3 sm:max-w-md">
              <div className="rounded-xl border border-hairline bg-page p-3 transition-transform duration-300 hover:-translate-y-0.5">
                <p className="font-auth-ui text-xs uppercase tracking-[0.14em] text-ink-muted">
                  Monthly
                </p>
                <p className="mt-1 font-heading text-2xl text-ink-display">
                  12+
                </p>
                <p className="text-xs text-ink-muted">Events coordinated</p>
              </div>
              <div className="rounded-xl border border-hairline bg-page p-3 transition-transform duration-300 hover:-translate-y-0.5">
                <p className="font-auth-ui text-xs uppercase tracking-[0.14em] text-ink-muted">
                  Resident reach
                ...
                <p className="text-xs text-ink-muted">Notified on time</p>
              </div>
            </div>
            ...
            <div className="auth-motion-item auth-motion-delay-4 mt-8 rounded-2xl border border-signal bg-signal-faded p-4 text-signal-display shadow-[0_18px_35px_-24px_rgba(15,23,42,0.45)] sm:p-5">
              ...
              <div className="mt-3 space-y-2 text-sm">
                <p className="flex items-center justify-between border-b border-signal/55 pb-2">
        ...
        <div className="flex items-center justify-center">
          <div className="auth-motion-card w-full max-w-xl rounded-[1.8rem] border border-hairline bg-page p-6 shadow-[0_26px_40px_-30px_rgba(15,23,42,0.35)] sm:p-8 lg:p-10">
            <div className="text-center lg:text-left">
              ...
              <p className="font-auth-ui text-xs uppercase tracking-[0.2em] text-ink-muted">
                Commons Portal Access
              </p>
              <h2 className="font-auth-display mt-2 text-3xl font-semibold tracking-tight text-ink-display sm:text-4xl">
                {title}
              </h2>
              {subtitle && (
            ...
            <div className="mt-7 border-t border-hairline pt-6">{children}</div>
          </div>
        </div>
      </div>
    </div>
