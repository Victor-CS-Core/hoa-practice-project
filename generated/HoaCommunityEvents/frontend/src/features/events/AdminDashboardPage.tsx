...
import type { CreateEventFormValues, EventFilter, HoaEvent } from "../../types/event";
...
export function AdminDashboardPage() {
  ...
  const [selectedAttendeesEventId, setSelectedAttendeesEventId] = useState<string | null>(null);
  ...
  const attendeesQuery = useAttendees(selectedAttendeesEventId ?? undefined, !!selectedAttendeesEventId);
  const hasActiveFilters = !!status || !!category;

  if (!authStore.isAdmin) {
    return (
      <section className="rounded-xl border border-amber-200 bg-amber-50 p-6">
        <h2 className="font-heading text-2xl font-bold text-amber-900">Admin Access Required</h2>
        <p className="mt-2 text-amber-800">This dashboard is available only to HOA administrators.</p>
        <p className="mt-4">
          <Link to="/events" className="text-amber-900 underline">Return to events</Link>
        </p>
      </section>
    );
  }
  ...
  const handleEdit = async (values: CreateEventFormValues) => {
    ...
    try {
      const updated = await editMutation.mutateAsync({ id: formState.event.id, values });
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      const next = toApiError(error);
      setActionError(next?.message ?? `Failed to ${confirmState.action} event.`);
    }
  };

  const isConfirmSubmitting = cancelMutation.isPending || deleteMutation.isPending;
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-3xl font-bold text-stone-900">Admin Management Dashboard</h1>
          <p className="mt-2 text-stone-600">Manage events, attendees, and publish updates for residents.</p>
        </div>
        <button
          type="button"
          onClick={() => {
      ...
      <div className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            value={status}
            onChange={(event) => updateFilter({ status: event.target.value || undefined, page: 1 })}
          ...
          <select
            value={category}
            onChange={(event) => updateFilter({ category: event.target.value || undefined, page: 1 })}
            className="rounded-md border border-stone-300 px-3 py-2 text-sm"
          >
          ...
          {hasActiveFilters && (
            <button
              type="button"
              onClick={() => updateFilter({ status: undefined, category: undefined, page: 1 })}
        ...
        {eventsQuery.data && (
          <p className="mt-2 text-xs text-stone-500">
            Total results: {eventsQuery.data.totalCount} • Page {eventsQuery.data.page} of {eventsQuery.data.totalPages}
          </p>
        )}
      </div>

      {inFlightMessage && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-amber-800">{inFlightMessage}</div>
      )}
      ...
      {eventsQuery.isLoading && (
        <div className="rounded-xl border border-stone-200 bg-white p-8 text-center text-stone-600">Loading events...</div>
      )}
      ...
      {!eventsQuery.isLoading && !eventsQuery.isError && eventsQuery.data && eventsQuery.data.items.length === 0 && (
        <div className="rounded-xl border border-stone-200 bg-white p-8 text-center text-stone-600">
          {status || category ? "No events match your filters." : "No events found."}
        </div>
      )}
      ...
      {eventsQuery.data && eventsQuery.data.items.length > 0 && (
        <AdminEventList
          ...
          onEdit={(eventId) => {
            const hit = eventsQuery.data?.items.find((item) => item.id === eventId);
            if (!hit) return;
            setFormError(null);
      ...
      {selectedAttendeesEventId && (
        <div className="rounded-xl border border-stone-200 bg-white p-4">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <h2 className="font-heading text-xl font-semibold text-stone-900">Attendees Panel</h2>
            <button
              type="button"
          ...
          <p className="mb-4 mt-1 text-sm text-stone-600">Viewing attendees for event ID: {selectedAttendeesEventId}</p>

          {attendeesQuery.isLoading && <p className="text-stone-500">Loading attendees...</p>}
          {attendeesQuery.isError && (
            <p className="rounded-lg border border-red-200 bg-red-50 p-3 text-red-700">Failed to load attendees.</p>
          )}
          {attendeesQuery.data && (
            <AdminAttendeeList attendees={attendeesQuery.data} totalCount={attendeesQuery.data.length} />
          )}
        </div>
      )}
          ...
          initialValues={
            formState.mode === "edit"
              ? {
                  ...
                  locationWithinCommunity: formState.event.locationWithinCommunity,
