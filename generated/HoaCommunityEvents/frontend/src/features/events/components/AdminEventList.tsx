    ...
    <div className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
              ...
              return (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="mb-1 font-semibold text-stone-900">{event.title}</div>
                    <div className="mt-2 flex items-center gap-3 text-xs text-stone-500">
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" /> {startDate}
                      </span>
                      <span className="flex items-center gap-1">
                        <MapPin className="h-3 w-3" /> {event.locationWithinCommunity}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <Badge
                      variant="secondary"
                      className={isCancelled ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}
                    >
                      {event.status}
                    </Badge>
                  </td>
      ...
      <div className="flex items-center justify-between border-t border-stone-200 bg-stone-50 px-6 py-4 text-sm text-stone-500">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" disabled={feed.page <= 1} onClick={() => onPageChange(feed.page - 1)}>
            Previous
          </Button>
          <Button
            variant="outline"
